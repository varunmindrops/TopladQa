...
<head>
    ...
    @if ($segment == 'cma' || $segment == 'cs')
        <title>Buy Combo Offer classes | June 2022 | Top Faculty</title>
    @elseif($segment == 'ca')
        <title>Buy Combo Offer classes | November 2021 | Top Faculty</title>
    @endif
...
<script>
    var options = {
        ...
        "notes": {
            ...
            "Group Name": "{{ $response['group'] }} Combo",
            "Subject And Faculty ID": '<?php echo htmlspecialchars_decode(json_encode($data)); ?>',
            "Delivery Mode": "{{ $response['saleMode'] }}",
    ...
    options.modal = {
        ondismiss: function() {
            if ($segment == 'cma' || $segment == 'cs') {
                location.href = "/{{ $segment }}/{{ $segment }}-combo-offer-june-2021-exam-classes";
            } else if ($segment == 'ca') {
                location.href = "/{{ $segment }}/{{ $segment }}-combo-offer-exam-classes";
            }
        },
...
<form action="/{{ $segment }}/payment/complete-pay" method="POST" hidden>
    @csrf
    ...
    <input type="text" class="form-control" name="teacher_id" id="teacher_id"
        value="{{ $response['teacherId'] }}">
    <input type="text" class="form-control" name="subject_id" id="subject_id"
        value="{{ $response['subjectId'] }}">
    <input type="text" class="form-control" name="group_name" id="group_name" value="{{ $response['group'] }}">
    <input type="text" class="form-control" name="delivery_mode" id="delivery_mode"
        value="{{ $response['saleMode'] }}">
