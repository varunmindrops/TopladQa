...
<head>
    ...
    <title>Buy Chapter Sale | June/December 2021 Exam</title>
...
var options = {
    ...
    "name": "Chapter Sale",
    "description": '<?php echo htmlspecialchars_decode($subject['name']) ?>',
    ...
    "notes": {
        ...
        "Chapter Id": "{{ $response['chapterId'] }}",
        "Faculty Id": "{{ $response['teacherId'] }}",
        "Amount(₹)": "{{ $response['chapterPrice'] }}",
...
options.modal = {
		ondismiss: function() {
            location.href="/{{$segment}}/{{$segment}}-chapter-sale";
		},
...
<form action="/{{$segment}}/payment/complete-chapter-order" method="POST" hidden>
    @csrf
    ...
    <input type="text" class="form-control" name="chapter_id" id="chapter_id" value="{{ $response['chapterId'] }}">
    <input type="text" class="form-control" name="teacher_id" id="teacher_id" value="{{ $response['teacherId'] }}">
    <input type="text" class="form-control" name="chapter_price" id="chapter_price" value="{{ $response['chapterPrice'] }}">
